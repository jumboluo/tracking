...
class Dataimport1Action extends Action{
	private static $refreshcookiefile = 'tracking_data_import1_refresh_cookie.txt';
	private static $tempcookiefile = 'tracking_data_import1_temp_cookie.txt';
	private static $base_url = 'http://t.aasop.com/';
	...
	public function login($returnOnSuccess=true, $startRefreshThreadAfterLogin=true) {
		...
		if($re['success'] && $re['response']==='1') {
			...
			if($startRefreshThreadAfterLogin) $this->_startRefreshThread();
	...
	public function logout() {
		...
		M()->execute("UPDATE `data_import1` SET `logined`='0'");
	...
	public function group() {
		$DataImportGroup = M('DataImportGroup1');
	...
	public function getgroups() {
		...
		M()->execute('TRUNCATE `data_import_group1`');
	...
	public function getnumbers() {
		...
		M()->execute("UPDATE `data_import1` SET `logined`='1', `last_login`='".date('Y-m-d H:i:s')."'");
		M()->execute('TRUNCATE `data_import_number1`');
	...
	private function _addNumber($number, $name, $group) {
		$data = array(
			...
			'group' => $group,
			'number' => $number,
			'last_time' => date('Y-m-d H:i:s', time()-3600*24) //重新导入将会重新获取一天以内的数据
		);
	...
	public function timing() {
		$DataImportTiming = M('DataImportTiming1');
	...
	public function gettimings() {
		...
		M()->execute("UPDATE `data_import1` SET `logined`='1', `last_login`='".date('Y-m-d H:i:s')."'");
		M()->execute('TRUNCATE `data_import_timing1`');
		...
		for($p=1; $p<=$pageCount; $p++) {
			...
			$name_pattern = "/<div style='width: 70px;' class='table_cell'>(.*)<\/div>/";
			preg_match_all($name_pattern	, $groupsHtml['response'], $names);
...
//		}
		...
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`starttime`=`data_import_timing1`.`starttime`, " .
				"`device`.`endtime`=`data_import_timing1`.`endtime`");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday0`=`data_import_timing1`.`weekday0` " .
				"WHERE `data_import_timing1`.`weekday0` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday1`=`data_import_timing1`.`weekday1` " .
				"WHERE `data_import_timing1`.`weekday1` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday2`=`data_import_timing1`.`weekday2` " .
				"WHERE `data_import_timing1`.`weekday2` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday3`=`data_import_timing1`.`weekday3` " .
				"WHERE `data_import_timing1`.`weekday3` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday4`=`data_import_timing1`.`weekday4` " .
				"WHERE `data_import_timing1`.`weekday4` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday5`=`data_import_timing1`.`weekday5` " .
				"WHERE `data_import_timing1`.`weekday5` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`weekday6`=`data_import_timing1`.`weekday6` " .
				"WHERE `data_import_timing1`.`weekday6` IS NOT NULL");
		
		M()->execute("UPDATE `device` INNER JOIN `data_import_timing1` ON `device`.`mobile_num`=`data_import_timing1`.`number` " .
				"SET `device`.`interval`=`data_import_timing1`.`interval`, " .
				"`device`.`delay`=`data_import_timing1`.`interval` " .
				"WHERE `data_import_timing1`.`interval` IS NOT NULL");
	...
	public function test() {
		$html = 'jierrorlat=40.57350;jierrorlon=110.02042;lat=40.57350;lon=110.02042;gpsdatetime=2012-11-23 09:28:35;note=内蒙古自治区包头市,站北路-一中西路交叉路口,黄河酒店,黄河饭店附近
;gpsdatetime=2012-11-23 09:28:35
...
<head><title>
	无标题页
		...
		$pattern ='/jierrorlat=(.*);jierrorlon=(.*);lat=(.*);lon=(.*);gpsdatetime=(.*);note=(.*)/';
		preg_match_all($pattern, $html, $locations);
		
		echo 'http://'.$_SERVER['HTTP_HOST'].U('Interface/gps');
	}
	
	//////////////////////////刷新///////////////////////////////
	public function startrefresh() {
		M()->execute("UPDATE `data_import1` SET `refresh_on`='1'");
	...
	public function stoprefresh() {
		M()->execute("UPDATE `data_import1` SET `refresh_on`='0', `refresh_heartbeat`=null");
		return_value_json(true);
	}
...
// 			$lastRefresh = time();
// 			do{
				M()->execute("UPDATE `data_import1` SET `refresh_heartbeat`='".date('Y-m-d H:i:s')."'");
				$setting = $this->_getSetting();
...
// 			while((time()-$lastRefresh<$setting['refresh_frequency']*60) && $setting['refresh_on']);
		...
		M()->execute("UPDATE `data_import_location1` SET `refreshing`='0' WHERE 1");
...
//		Log::write("\nREFRESHING logined, threadId=".$threadId, Log::INFO);
		...
		M()->execute("UPDATE `data_import1` SET `refresh_finished`='".date('Y-m-d H:i:s')."'");
		M()->execute("UPDATE `data_import_location1` SET `refreshing`='0' WHERE 1");
		return true;
	}
	...
	private function _gerNumbersForRefreshing($ids=null) {
		...
		$condition = empty($ids) ? '1' : "`data_import_number1`.`id` IN (".$ids.")";
		
		$numbers = $DataImportNumber->join('`device` on `device`.`mobile_num`=`data_import_number1`.`number`')
			->field(array('`device`.`id`' => 'device_id', '`data_import_number1`.`number`', 
						'`data_import_number1`.`name`', '`data_import_number1`.`last_time`', '`device`.`label`'))
			->where($condition)
			->order('`data_import_number1`.`id`')
			->select();
		check_error($DataImportNumber);
		
		M()->execute('TRUNCATE `data_import_location1`');
	...
	private function _refreshNumber($number, $threadId, $checkThreadId=true) {
		...
		if($checkThreadId && !$setting['refresh_on']) {
			M()->execute("UPDATE `data_import1` SET `refresh_heartbeat`=null");
			return false;
		}
		...
		M()->execute("UPDATE `data_import1` SET `refresh_heartbeat`='".date('Y-m-d H:i:s')."', `refresh_thread_id`='{$threadId}'");
		...
		if($number['last_time']=='0000-00-00 00:00:00') {	//如果时间不正确，则纠正时间。
			...
			M()->execute("UPDATE `data_import_number1` SET `last_time`='".$number['last_time']."' WHERE `number`='{$number['number']}'");
		}
...
// 		Log::write("\nlocationHtml=" . print_r($locationHtml, true), Log::INFO);
		...
		M()->execute("UPDATE `data_import1` SET `refresh_heartbeat`='".date('Y-m-d H:i:s')."', `last_refresh`='".date('Y-m-d H:i:s')."', `refresh_thread_id`='{$threadId}'");
		if(!empty($location['time'])){
			M()->execute("UPDATE `data_import_number1` SET `last_time`=DATE_ADD('".$location['time']."', INTERVAL 1 SECOND) WHERE `number`='{$number['number']}'");
		}
		return true;		
	}
...
// 		Log::write(M()->getLastSql(), Log::SQL);
		...
		$this->_addGpsLocation($location);
	}
	
	private function _addGpsLocation($location) {
...
//		if(empty($ids)) return_value_json(false, 'msg', '系统错误：提交的分组id为空');
		...
		if(function_exists('fsockopen')) { //可以用fsockopen
			...
			//发送get
			$crlf = "\r\n"; 
			$file = U('Dataimport1/dorefreshselected');
...
//			Log::write("\n_startRefreshThread last refreshing", Log::INFO);
		...
		if(function_exists('fsockopen')) { //可以用fsockopen
			...
			//发送get
			$crlf = "\r\n"; 
			$file = U('Dataimport1/refreshthread');
			$header = "POST $file HTTP/1.1" . $crlf;
			$header .= "Host: {$_SERVER['HTTP_HOST']}" . $crlf;
			$header .= 'Content-Type: application/x-www-form-urlencoded' . $crlf; 
	...
	private function _getSetting() {
		...
		if(!empty($setting) && !empty($setting[0]['last_login'])) {
			if(time()-strtotime($setting[0]['last_login'])>10*60) {	//假设客户端的session为10分钟（待测试）
				$setting[0]['logined'] = 0;
				$DataImport->execute("UPDATE `data_import1` SET `logined`='0'");
			}
		}
		if(!empty($setting)) {
	...
	public function getlatestlocation() {
		$req_numbers = $this->_request('numbers');
		if(empty($req_numbers)) {
			return_value_json(true);
		}
		
		$numbers = explode(',', $req_numbers);
		$result = '';
		foreach ($numbers as $number) {
			$ret = $this->_getLatestLocation($number);
			if($ret!==true){
				$result .= '定位手机['.$number.']时出错：'.$ret.'<br/>';
			}
		}
		
		if(!empty($result)) {
			return_value_json(false, 'msg', $result);
		}
		else {
			return_value_json(true);
		}
	...
	private function _getLatestLocation($num) {
		if(empty($num)) {
			return true;
		}
		
		$DataImportNumber = M('DataImportNumber1');
		$number = $DataImportNumber->join('`device` on `device`.`mobile_num`=`data_import_number1`.`number`')
		->field(array('`data_import_number1`.`number`', '`device`.`label`'))
		->where("`device`.`mobile_num`='{$num}'")
		->find();
		
		if(empty($number)) {
			return '即时定位暂时只支持手机定位数据导入方式，您现在看到的定位信息也许有时间延迟';
		}
...
// 		Log::write("\nlocationHtml=" . print_r($locationHtml, true), Log::INFO);
		...
		if(empty($locationHtml['success'])) {
			return '立刻定位获取数据失败，请检查网络连接';
		}
			
		$pattern ='/jierrorlat=(.*);jierrorlon=(.*);lat=(.*);lon=(.*);gpsdatetime=(.*);note=(.*)/';
		preg_match_all($pattern, $locationHtml['response'], $locations);
			
		if(count($locations)<7) {
			return '立刻定位得到的结果不对，也许数据来源网站的结果数据结构发生了变化，请联系本系统的开发人员。';
		}
			
		if(empty($locations[3][0]) && empty($locations[4][0])) {
			return strstr($locations[6][0], ';', true);
		}
			
		$location = array(
				'label' => $number['label'],
				'latitude' => $locations[3][0],
				'longitude' => $locations[4][0],
				'address' => $locations[6][0],
				'time' => $locations[5][0]);
// 		Log::write("\nlocation=" . print_r($location, true), Log::INFO);
