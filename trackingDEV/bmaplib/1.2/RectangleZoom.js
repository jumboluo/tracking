...
(function() {
    ...
    RectangleZoom.prototype.open = function() {
        ...
        // 已有其他地图上的鼠标操作工具开启
        if (!this._map || !!this._map._toolInUse) { //原来：!!BMapLib._toolInUse，修改by Jumboluo 2012/4/22
            return;
        }
        this._isOpen = true;
        ...
        // 增加鼠标按下时，开始绘制矩形框的事件
        // 通过判断只绑定一次，并不再删除
        // by JumboLuo: 不再删除是个垃圾idea，如果有多个控件实例，某些实例创建后又删除了，但是事件不删除，将使未删除的实例也无法开始画矩形
        //
        if (!this._isBeginDrawBinded) {
            addEvent(OperationMask.getDom(this._map), "mousedown", beginDrawRect);
            this.beginDrawRect = beginDrawRect; //就这样修修补补算了！ by JumboLuo
    ...
    RectangleZoom.prototype.close = function() {
        ...
        this._isOpen = false;
        if(this._map) this._map._toolInUse = false;
        this._followTitle && this._followTitle.hide();
    	if(!!this._isBeginDrawBinded) {
    		removeEvent(OperationMask.getDom(this._map), "mousedown", this.beginDrawRect);
    		this._isBeginDrawBinded = false;
    	}
        OperationMask.hide();
    };
    
    RectangleZoom.prototype.remove = function() {
    	this.close();
    	OperationMask.remove();
    };
    ...
    var OperationMask = {
        ...
        getDom : function(map) {
            ...
            if(this._map != map) {
            	this._map = map;
            	this._maskElement = map.getContainer().lastChild;
            }
